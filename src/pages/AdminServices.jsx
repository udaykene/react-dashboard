    ...
    <AdminLayout>
      <div className="flex h-screen flex-col overflow-y-auto no-scrollbar pb-6">
        ...
        <div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
          <div>
            <h1 className="m-0 text-[2rem] font-bold text-[#1e293b]">Services</h1>
        ...
        {/* Filters Section */}
        <div className="mb-6 flex flex-col gap-4 rounded-[12px] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.05)] lg:flex-row lg:items-center lg:justify-between">
          {/* Category Filter */}
          <div className="flex flex-col gap-3">
            <label className="text-[0.875rem] font-semibold text-[#64748b]">Category</label>
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  key={category.name}
                  onClick={() => setFilterCategory(category.name)}
                  className={`rounded-lg border-2 px-5 py-2.5 text-[0.875rem] font-semibold transition-all ${
          ...
          <div className="flex flex-col gap-3 lg:items-end">
            <label className="text-[0.875rem] font-semibold text-[#64748b]">Status</label>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filteredServices.length === 0 ? (
            <div className="col-span-full rounded-[12px] bg-white px-8 py-16 text-center shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
              <i className="ri-service-line mb-4 text-[4rem] text-[#cbd5e1]"></i>
            ...
            filteredServices.map((service) => (
              <div
                key={service.id}
                className="group flex flex-col rounded-[12px] border-t-4 bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.05)] transition-all duration-300 hover:translate-y-[-4px] hover:shadow-[0_8px_20px_rgba(0,0,0,0.1)]"
              ...
              >
                ...
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <span
                    className={`rounded-md px-3 py-1.5 text-[0.75rem] font-semibold capitalize ${getStatusStyles(
                      service.status
                    )}`}
                  >
                    {service.status}
                ...
                <div className="mb-4 flex-1">
                  <h3 className="m-0 mb-2 text-[1.125rem] font-bold text-[#1e293b]">
                    {service.name}
                  </h3>
                  <p className="m-0 mb-3 text-[0.875rem] leading-relaxed text-[#64748b]">
                    {service.description}
                  </p>
                  <div className="mb-3 inline-block rounded-md bg-[#f1f5f9] px-3 py-1 text-[0.75rem] font-semibold text-[#475569]">
                    {service.category}
                  </div>
                </div>

                {/* Service Meta */}
                <div className="mb-4 flex flex-col gap-3 border-t-2 border-[#f1f5f9] pt-4">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-[0.875rem] text-[#64748b]">
                      <i className="ri-money-dollar-circle-line text-[1rem] text-[#94a3b8]"></i>
                      Pricing
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-[0.875rem] text-[#64748b]">
                      <i className="ri-time-line text-[1rem] text-[#94a3b8]"></i>
                      Duration
                    </span>
                    <span className="text-[0.875rem] font-bold text-[#1e293b]">
                      {service.duration}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-[0.875rem] text-[#64748b]">
                      <i className="ri-user-line text-[1rem] text-[#94a3b8]"></i>
                      Clients
                    </span>
                    <span className="text-[0.875rem] font-bold text-[#1e293b]">
                      {service.clients}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-[0.875rem] text-[#64748b]">
                      <i className="ri-star-fill text-[1rem] text-[#fbbf24]"></i>
                      Rating
                    </span>
                    <span className="text-[0.875rem] font-bold text-[#1e293b]">
                      {service.rating} / 5.0
                ...
                <div className="flex gap-2">
                  <button className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#dbeafe] px-4 py-2.5 text-[0.875rem] font-semibold text-[#1e40af] transition-all hover:scale-105 hover:bg-[#3b82f6] hover:text-white">
                    <i className="ri-eye-line"></i>
                    View
                  </button>
                  <button className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#fef3c7] px-4 py-2.5 text-[0.875rem] font-semibold text-[#92400e] transition-all hover:scale-105 hover:bg-[#f59e0b] hover:text-white">
                    <i className="ri-edit-line"></i>
                    Edit
                  </button>
                  <button
                    onClick={() => handleStatusToggle(service.id)}
                    className={`flex items-center justify-center rounded-lg px-3 py-2.5 text-[1.125rem] transition-all hover:scale-110 ${
        ...
        <div className="mt-6 rounded-[12px] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div
                key={index}
                className="flex items-center gap-4 rounded-lg bg-[#f8fafc] p-4 transition-all hover:translate-x-1 hover:bg-[#f1f5f9]"
              >
                <div
                ...
                <div className="flex-1">
                  <h4 className="m-0 text-[1rem] font-bold text-[#1e293b]">{cat.name}</h4>
